...
```
createuser -P clicktrack
createdb -E utf8 clicktrack -O clicktrack
```
...
Alternatively, copy the `clicktrack/settings/db.py.base` to
`clicktrack/settings/db.py` and modify it to your needs,
...
Alternatively, copy the `clicktrack/settings/email.py.base` to
`clicktrack/settings/email.py` and modify it to your needs,
then run the rest of the commands.
...
```
docker-compose exec clicktrack-backend bash
```
